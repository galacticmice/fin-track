...
public class ActivityDAO {
    public static boolean addActivity(Activity activity) {
        try (Connection conn = ConnectDB.getConnection()) {
            String query = "INSERT INTO Activity (entry_time, user_id, category_id, type, amount, description, recur_days) VALUES (?, ?, ?, ?, ?, ?, ?)";
            PreparedStatement ps = conn.prepareStatement(query);
            ps.setTimestamp(1, activity.getEntryTime());
            ps.setInt(2, activity.getUser_id());
            ps.setInt(3, activity.getCategory_id());
            ps.setString(4, String.valueOf(activity.getType()));
    ...
    public static boolean updateActivity(int activity_id, Activity updated_activity) {
        try (Connection conn = ConnectDB.getConnection()) {
            ...
            ps.setString(4, String.valueOf(updated_activity.getType()));
