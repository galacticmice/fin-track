...
public class ActivityDAO {
    public static boolean addActivity(Activity activity) {
        try (Connection conn = ConnectDB.getConnection()) {
            String query = "INSERT INTO Activity (entry_time, user_id, category_id, activity_type, amount, description) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public static boolean updateActivity(int activity_id, Activity updated_activity) {
        try (Connection conn = ConnectDB.getConnection()) {
            String query = "UPDATE Activity SET entry_time=?, category_id=?, type=?, amount=?, description=? WHERE id=?";
            PreparedStatement ps = conn.prepareStatement(query);
